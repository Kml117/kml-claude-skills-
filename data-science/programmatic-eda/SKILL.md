...
3. **Outlier detection** — run `scripts/outlier_detector.py` (IQR + z-score) on numeric columns; document flagged values and decide: real signal or data error?
   - ⚠️ The script's z-score is classic (mean/std). If the distribution is visibly skewed (check step 4 first), classic z-score is unreliable — the skill `data-cleaning-quality` has the modified z-score (median + MAD) approach for that case; don't trust this script's z-score column blindly on skewed data.
...
5. **Correlation exploration** — run `scripts/correlation_explorer.py`; flag pairs with |r| > 0.8 as potential multicollinearity or redundancy.
   - ⚠️ This script only computes Pearson/Spearman/Kendall on numeric columns — it silently skips categorical variables entirely. If the dataset has categorical columns that matter, this alone is not a complete correlation picture. The skill `eda-feature-engineering` has the decision map for Phik/Cramér's V on mixed/categorical data — use it before concluding "no correlation" just because this script found none.
